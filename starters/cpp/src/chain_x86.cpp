...
// 2026-08-22): all 21 selftest digest checks pass (FIPS vectors, short
// chains, both full 50,000-round chains, every x2-interleaved variant
// including swapped lanes), and it measured ~4.0x the OpenSSL-per-call
// reference path (16.7ms -> 4.2ms per chain, three repeated trials, same
// binary, same run -- see STRATEGY.md section 12). It is now auto-selected,
// the same as arm_crypto_backend().
//
...
// for the selftest binary itself). A wrong digest here gets rejected and the
// chain falls back to the reference path at runtime -- but tests/
// selftest.cpp treats that rejection as a build failure on purpose, so
// re-verify before merging any change to this file.
...
void chain2_impl(const char in_a[64], const char in_b[64], int rounds,
  ...
  // Coarse-grained interleaving: two independent, data-dependency-free calls
  // back to back let an out-of-order core overlap some of sha256rnds2's
  // multi-cycle latency between the two chains. Not as tight a weave as the
  // ARM back end's round-by-round interleave -- that would mean carrying two
  // Lane states through one hand-unrolled loop, doubling the amount of this
  // file that is unverified. Worth revisiting once this baseline is confirmed
...
const Backend kX86Backend = {"x86-sha-ni (verified, x2 interleaved)",
                             chain1_impl, chain2_impl};
