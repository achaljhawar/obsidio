...
// The only endpoint with a body is POST /price and its JSON is ~50 bytes.
// Anything larger is not a client of this API.
...
// Test-only SO_SNDBUF pin, 0 when unset. See the use site in accept_new().
...
struct Server::Connection {
  ...
  bool closed{false};
  // The mask currently registered with epoll, 0 when the fd is out of the set.
  // Lets arm() skip epoll_ctl when the interest set is already correct, which
  // is every successful flush on a keep-alive connection.
  std::uint32_t armed{};
};
...
struct Server::Loop {
  ...
  // Connection events carry their Connection* in data.ptr, so the two fds with
  // no Connection carry these addresses instead. Compared by address only.
  char listen_tag{};
  char wake_tag{};
...
bool Server::bind_listener(Loop& loop) {
  ...
  // Each IO thread gets its own accept queue, so there is no shared accept lock.
#ifdef SO_REUSEPORT
...
bool Server::start() {
  ...
  for (std::size_t i{}; i < io_threads_; ++i) {
    ...
    // The Loop is heap-allocated and never moves, so its tag addresses stay
    // valid even though the owning unique_ptr moves into loops_ below.
    epoll_event ev{};
    ev.events = EPOLLIN;
...
void Server::loop_main(std::size_t index) {
  ...
  // Make `wanted` this connection's interest set, doing nothing when that is
  // already registered -- the common case, since flush() ends every keep-alive
  // request wanting the EPOLLIN it already has.
  ...
  // Connections torn down during the current batch. Reclaiming one immediately
  // is unsafe now that events carry raw pointers: epoll_wait already
  // snapshotted this batch, so a later event can name a connection an earlier
  // one killed. Holding the fd open until the batch drains also stops accept4()
  // reusing the descriptor and aliasing a stale event onto a live connection.
  ...
  auto flush = [&](Connection* conn) -> bool {
    ...
    // Usually a no-op: EPOLLOUT is only armed by the partial-write branch.
    arm(conn, static_cast<std::uint32_t>(EPOLLIN));
    return true;
  };

  // Answer a protocol error and close. Framing is unknown past this point, so
  // the connection is never kept alive after a reject.
  ...
  auto process = [&](Connection* conn) -> bool {
    for (;;) {
      ...
      // Headers one line at a time, never a substring hunt over the block:
      // that would match "X-Content-Length:", miss mixed case, and silently
      // take the first of two conflicting values. Content-Length is the
      // request's framing, so it is parsed strictly or the request dies.
      bool cl_seen{false};
      ...
      while (cursor < head.size()) {
        ...
        // Whitespace inside a header name is a classic smuggling shape: one
        // parser sees the header, another does not.
        ...
        if (iequals(name, "content-length")) {
          ...
          for (const char c : value) {
            ...
            if (!too_large) {
              // Saturating: past the cap the exact value stops mattering, and
              // the loop can never overflow size_t.
          ...
          // Accepting this while reading Content-Length bytes is exactly how
          // request smuggling works, so it is a hard reject.
          error_status = 400;
      ...
      // Subtraction, never body_start + content_length: that sum was the
      ...
      if (!answered) {
        // A worker owns this connection now; take it out of the epoll set until
        // complete() hands it back.
        conn->deferred = true;
        disarm(conn);
        return true;
      }
      ...
      conn->out_sent = 0;
      // Only flush() may decide a connection is finished: returning false on
      // close_after here would tear down a partially drained response.
  ...
  auto accept_new = [&]() {
    for (;;) {
      ...
      // Test hook, unset in production. The kernel autotunes the send buffer to
      // megabytes, so the partial-write path is unreachable over loopback;
      // pinning the buffer small (which also disables autotuning) makes
      // backpressure reproducible in tests/http_test.cpp.
      if (forced_sndbuf > 0) {
      ...
      // arm() before the map takes ownership, so a failed ADD costs only the fd.
  ...
  auto drain_completions = [&]() {
    ...
    for (auto& done : batch) {
      ...
      // EPOLLOUT. process() disarmed on defer, so this is an ADD.
      arm(conn, static_cast<std::uint32_t>(EPOLLIN));

      // flush() returns false once a close_after response has fully drained;
      // checking close_after here too would truncate a partial write.
      if (!flush(conn)) {
  ...
  while (running_.load(std::memory_order_relaxed)) {
    ...
    for (int i{}; i < n; ++i) {
      ...
      // hash of its fd. Safe against stale pointers because close_connection()
      ...
      if (events[i].events & EPOLLOUT) {
        ...
        // Draining may have unblocked requests already read into conn->in
        // behind the response. epoll is level triggered on the socket and those
        // bytes have left it, so without parsing here, pipelined requests
        // behind a backpressured write are never answered at all.
    ...
    // End of batch: no pointer from `events` is live, so closed connections can
    // be freed and their descriptors released for reuse.
  ...
  // Teardown: deferred connections are closed by their completion path being
  // dropped, which is fine at shutdown.
