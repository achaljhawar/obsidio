...
# Mean container CPU% across the probe window. This is what converts a
# throughput number into a cost-per-request: 200% means both granted CPUs were
# busy, so requests/sec divided by CPU-seconds/sec gives CPU per request.
sample_cpu() {
  local out="$1"
  : > "$out"
  while :; do
    docker stats --no-stream --format '{{.CPUPerc}}' "$CTR" 2>/dev/null \
      | tr -d '%' >> "$out"
    sleep 2
  done
}
...
for rep in $(seq 1 "$REPS"); do
  for arm in "$@"; do
    ...
    cpu_mean=$(awk 'NF{s+=$1;n++} END{if(n)printf "%.1f",s/n; else print "0.0"}' "$cpu_file")
    rm -f "$cpu_file"
    ...
    if [ "$PROBE" = "risk" ]; then
      metric=$(field "$out" rate); label="rate"; extra_col=""
    else
      metric=$(field "$out" score); label="score"
      # The scheduling sweep is decided by score AND price latency together, so
      # the fast-path number has to be on the same line as the throughput.
      extra_col="  price_p95=$(field "$out" price_p95)ms"
    fi
    ...
    printf "rep=%d arm=%-10s %s=%-12s fail=%s%%  cpu=%s%%%s  clock=%s%%  temp=%s%s\n" \
      "$rep" "$name" "$label" "$metric" "$fail" "$cpu_mean" "$extra_col" \
      "$drift" "$temp" "$warn"
    echo "$name $metric $fail $cpu_mean" >> "$RESULTS_FILE"
  done
done
...
for line in open(sys.argv[1]):
    ...
    if len(p) >= 4:
        cpus.setdefault(p[0], []).append(float(p[3]))
if not rows:
    print("no results"); sys.exit(1)

print(f"{'arm':<12}{'n':>3}  {'mean':>12}  {'spread%':>8}  {'max fail%':>10}  {'cpu%':>7}")
print("-" * 62)
means = {}
for name, vals in rows.items():
    ...
    means[name] = mean
    cpu = statistics.fmean(cpus[name]) if cpus.get(name) else 0.0
    print(f"{name:<12}{len(vals):>3}  {mean:>12.3f}  {spread:>7.2f}%  "
          f"{max(fails[name]):>9.3f}%  {cpu:>6.1f}%")
