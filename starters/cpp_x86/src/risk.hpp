...
// The unaccelerated chain, exposed so the self-test can check it against the
// golden digests directly. Without this the fallback path is only ever
// exercised on short chains inside back-end verification, and a bug in it would
// not surface until the day the accelerated back end failed to qualify.
std::string risk_hash_reference(const std::string& seed,
                                int iterations = kRiskIterations);
